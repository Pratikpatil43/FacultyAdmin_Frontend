    ...
    const fetchAttendance = async () => {
        ...
        try {
            const response = await axios.get(
                `http://localhost:5000/api/faculty/attendance/getAttendance?branch=${branch}&className=${className}&subject=${subject}&attendanceDate=${attendanceDate}`,
                { headers: { 'Authorization': `Bearer ${token}` } }
            );
    ...
    const updateStatus = async (recordId, newStatus) => {
        ...
        try {
            const response = await axios.put(
                `http://localhost:5000/api/faculty/attendance/updateAttendance/${recordId}`,
                { status: newStatus },
                { headers: { 'Authorization': `Bearer ${token}` } }
            );
